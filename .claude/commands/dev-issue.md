...
| Worktrees | Sibling directories `gvns.ca-issue-<N>` |
...
# Worktrees
git worktree add ../gvns.ca-issue-<N> -b <type>/<N>-<desc>
git worktree list
git worktree remove ../gvns.ca-issue-<N>
```
...
| Purpose | Query |
|---------|-------|
| Recent context | `get_recent_context` with project="gvns.ca", limit=30 |
...
Use `get_recent_context` for project="gvns.ca", limit=30.
...
```bash
gh issue list -R ggfevans/gvns.ca --state open --label ready \
...
Check for more ready issues: `gh issue list -R ggfevans/gvns.ca --state open --label ready --json number | jq 'length'`

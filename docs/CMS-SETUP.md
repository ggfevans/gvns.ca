...
Heroes are served through **Cloudflare Image Transformations** at render time, not pre-encoded by Sharp at build time. The pipeline:
...
The `@astrojs/cloudflare` adapter is configured with `imageService: "custom"` so it doesn't override `image.service`. If you ever swap unpic out, flip that back to `"compile"` (Sharp) or `"cloudflare"` (the adapter's built-in CF Image Transformations service).

...
Use **one primary** + 1–3 secondary tags. Kebab-case only. The Sveltia CMS tag field is a free-form `list` (min 1, max 4) — type any tag; the taxonomy below is the *recommended* set (shown in the field hint), not a hard constraint (the build validates count, not values).
